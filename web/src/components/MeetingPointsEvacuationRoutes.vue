<template >
  <div style="height: auto; width: 70%; margin: auto">
    ...
    <p v-if="!fetchedMeetingPoints || !fetchedEvacuationRoutes">
      Cargando mapa...
      <img style="height: 50px; width: 70px" src="../assets/loading.gif" />
    </p>
    ...
    <l-map :zoom="zoom" :center="center" style="height: 500px">
      ...
      <l-marker
        :lat-lng="
          this.userLatLong
            ? [this.userLatLong.lat, this.userLatLong.long]
            : this.center
        "
      >
        <l-icon
          :icon-url="require('../assets/initial_location.png')"
          :icon-size="[20, 30]"
        />

        <l-popup :options="{ maxHeight: 350 }">
          <strong>Mi ubicación</strong>
          {{ this.userLatLong ? "(GPS)" : "(default)" }}
        </l-popup>
      </l-marker>

      <!-- Se dibujan los puntos de encuentro -->
      <l-marker
        v-for="(meetingPoint, index) in meetingPoints.puntos_encuentro"
        :key="index"
        :lat-lng="[meetingPoint.coordenada.lat, meetingPoint.coordenada.long]"
      >
        <l-icon
          :icon-url="require('../assets/meeting_point.png')"
          :icon-size="[30, 40]"
        />
      ...
      <l-polyline
        v-for="(evacuationRoute, index) in evacuationRoutes.recorridos"
        :key="index"
        :lat-lngs="getCoordinateList(evacuationRoute.coordenadas)"
        :color="randomColor()"
      >
        <!-- Marcador de inicio del recorrido -->
        <l-marker
          :lat-lng="[
            evacuationRoute.coordenadas[0].lat,
            evacuationRoute.coordenadas[0].long,
          ]"
        >
          <l-icon
            :icon-url="require('../assets/evacuation_route_start.png')"
            :icon-size="[35, 45]"
          />
        ...
        <l-marker
          :lat-lng="getEvacuationRouteEndPoint(evacuationRoute.coordenadas)"
        >
          <l-icon
            :icon-url="require('../assets/evacuation_route_end.png')"
            :icon-size="[20, 30]"
          />
        </l-marker>
      </l-polyline> </l-map
    ...
    <div class="container">
      <div class="row">
        <div class="col-sm">
          ...
          <p v-if="!fetchedMeetingPoints">
            Cargando puntos de encuentro...<img
          ...
          <nav
            v-if="meetingPoints.puntos_encuentro.length"
            aria-label="Meeting points page navigation"
          >
            <ul class="pagination justify-content-center">
              ...
              <li
                v-for="page in [...Array(meetingPoints.paginas).keys()]"
                :key="page"
                class="page-item"
              ...
              <li
                v-if="meetingPoints.pagina < meetingPoints.paginas"
                class="page-item"
              >
                <a
                  class="page-link"
        ...
        <div class="col-sm">
          ...
          <table v-if="fetchedEvacuationRoutes" class="table table-striped">
            <thead>
              <tr>
                <th>
                  {{
                    evacuationRoutes.recorridos.length
                      ? "Nombre"
                      : "No se encontraron recorridos de evacuación"
                  }}
                </th>
              </tr>
            </thead>
          ...
          <p v-if="!fetchedEvacuationRoutes">
            Cargando recorridos de evacuación...<img
              style="height: 50px; width: 70px"
              src="../assets/loading.gif"
            />
          </p>
          ...
          <nav
            v-if="evacuationRoutes.recorridos.length"
            aria-label="Evacuation route page navigation"
          >
            <ul class="pagination justify-content-center">
              ...
              <li
                v-for="page in [...Array(evacuationRoutes.paginas).keys()]"
                :key="`page-${page}`"
                class="page-item"
              ...
              <li
                v-if="evacuationRoutes.pagina < evacuationRoutes.paginas"
                class="page-item"
              >
                <a
                  class="page-link"
  ...
  methods: {
    ...
    getEvacuationRouteEndPoint(coordinates) {
      ...
      return [
        ...
        coordinates[coordinates.length - 1].long,
      ];
    },
